...
class PhotoLifetimeTrackerSupabaseUser extends BaseAuthUser {
  PhotoLifetimeTrackerSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> photoLifetimeTrackerSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PhotoLifetimeTrackerSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
